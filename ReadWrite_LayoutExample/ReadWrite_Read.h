// ReadWriter_Read.h   simple stream read implementation example
...
auto read(std::ifstream& is, Pod& pod) -> std::enable_if_t<std::is_pod_v<Pod>>
{
...
void read(std::ifstream& is, std::string& str)
{
	...
	read(is, len);
	str.resize(len);
	is.read(data(str), len);
}

template <typename T>
void read(std::ifstream& is, std::vector<T>& vec)
{
	decltype(size(vec)) len;
	read(is, len);
	if (len == 0) return;
	do
	{
		read(is, vec.emplace_back());
	}
	while (--len);
}